...
async def post_forwarded_message(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=button_text, url=deep_link)]])
...
async def handle_post_deep_link(message: Message, raw_args: str):
    ...
    if not target_channel:
        # Fallback to direct link if no channels exist
        link = f"https://t.me/{username}/{message_id}"
        keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬇️ Download Episode", url=link)]])
        return await message.answer("Here is your requested episode:", reply_markup=keyboard)
    ...
    try:
        ...
        if member.status in ['left', 'kicked']:
            ...
            if not invite_link:
                try:
                    invite_link = await bot.export_chat_invite_link(req_channel_id)
                except Exception as e:
            ...
            return await message.answer("<b>Join Required!</b>\n\nTo access this episode, you must first join our channel. 👇", reply_markup=keyboard, parse_mode=ParseMode.HTML)
        else:
            link = f"https://t.me/{username}/{message_id}"
            keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬇️ Download Episode", url=link)]])
            return await message.answer("Thank you for being a member! Here is your episode:", reply_markup=keyboard)
    except Exception as e:
...
async def handle_start(message: Message, command: CommandStart):
    ...
    # --- Logic for /post command deep links ---
...
async def handle_random_post_join(callback: CallbackQuery):
    ...
    if not row:
        return await callback.answer("Channel no longer exists.", show_alert=True)
        
    req_channel_id = row['channel_id']
